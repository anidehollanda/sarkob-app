...
# =========================
# Umbrales (criterios clínicos)
# =========================
@dataclass
class Thr:
    handgrip_low = {"female": 16.0, "male": 27.0}  # kg
    alm_h2_low   = {"female": 5.5,  "male": 7.0 }  # kg/m²
    gait_slow_mps = 0.8
    chair5_slow_s = 15.0
    calf_low_cm   = 31.0
    sarcf_pos_cut = 4
    smmwt_low_pct = 35.0  # criterio operativo simple para %SMM/peso
...
def label_strength(hand_kg: Optional[float], sex: str) -> Dict[str, Any]:
    if hand_kg is None: return {"label": "indeterminado", "is_low": False}
    low = hand_kg < T.handgrip_low[sex]
    return {"label": ("baja" if low else "normal"), "is_low": low}

def label_calf(calf_cm: Optional[float]) -> Dict[str, Any]:
    if calf_cm is None: return {"label":"indeterminado","is_low":False}
    low = calf_cm < T.calf_low_cm
    return {"label": ("baja" if low else "normal"), "is_low": low}

def label_mass_almh2(alm_h2: Optional[float], sex: str) -> Dict[str, Any]:
    if alm_h2 is None: return {"label":"indeterminado","is_low":False}
    low = alm_h2 < T.alm_h2_low[sex]
    return {"label": ("baja" if low else "normal"), "is_low": low}
...
def overall_classification(strength_low: bool, mass_low: bool, gait: Optional[Dict[str,Any]], chair5: Optional[float]) -> str:
    status = "no sarcopenia"
    if strength_low and not mass_low:
        status = "sarcopenia probable"
    if strength_low and mass_low:
        status = "sarcopenia confirmada"
        slow_gait = bool(gait and gait.get("speed_mps", 1e9) < T.gait_slow_mps)
        slow_chair = (chair5 is not None and chair5 > T.chair5_slow_s)
        if slow_gait or slow_chair:
            status = "sarcopenia severa"
    return status
...
with cc3:
    vat_sat = st.number_input("Cociente VAT/SAT", min_value=0.0, value=0.0, step=0.01, format="%.2f")

cc4,cc5,cc6 = st.columns(3)
with cc4:
    smm_kg = st.number_input("SMM (kg)", min_value=0.0, value=0.0, step=0.1)
with cc5:
    alm_h2 = st.number_input("ALM/altura² (kg/m²)", min_value=0.0, value=0.0, step=0.1)
with cc6:
    calf_cm = st.number_input("Perímetro de pantorrilla (cm)", min_value=0.0, value=0.0, step=0.1)

mass = label_mass_almh2(alm_h2 if alm_h2>0 else None, sex_in)
calf = label_calf(calf_cm if calf_cm>0 else None)

# Cálculos dependientes
smm_pct = (smm_kg/weight_kg*100.0) if (smm_kg>0 and weight_kg>0) else None
gait_label = None
gait_speed = None
if gait:
    gait_label = "lenta" if gait["is_slow"] else "normal"
    gait_speed = gait["speed_mps"]

# Percentil de prensión
perc = handgrip_percentile(hand_kg if hand_kg>0 else None, sex_in, age_in)

# Clasificación global automática
overall = overall_classification(strength["is_low"], mass["is_low"], gait, chair5_s if chair5_s>0 else None)
...
cut_gait = "< 0.8 → lenta"
cut_chair = "> 15 → lenta"
cut_smm = f"< {T.smmwt_low_pct:.0f}% → baja"
cut_calf = f"< {T.calf_low_cm:.0f} → baja"
...
row_data = {
    ...
    "Valor": [
        pf(hand_kg if hand_kg>0 else None),
        ("" if perc is None else f"{perc:.1f}%"),
        ("" if gait_speed is None else f"{gait_speed:.2f}"),
        ("" if not chair5_s else f"{chair5_s:.1f}"),
        ("" if smm_pct is None else f"{smm_pct:.1f}%"),
        ("" if not calf_cm else f"{calf_cm:.1f}"),
        f"{sarcf['score']}",
    ],
    "Punto de corte / normalidad": [
        cut_hand,
        ("P50 ~ referencia por edad/sexo" if perc is not None else ""),
        cut_gait if gait_speed is not None else "",
        cut_chair if chair5_s else "",
        cut_smm if smm_pct is not None else "",
        cut_calf if calf_cm else "",
        f"≥ {T.sarcf_pos_cut} → positivo",
    ],
    "Interpretación": [
        (strength["label"] if hand_kg>0 else ""),
        ("muy bajo" if perc is not None and perc < 5 else ("bajo" if perc is not None and perc < 25 else ("normal" if perc is not None else ""))),
        (gait_label or ""),
        ("lenta" if chair5_s and chair5_s > T.chair5_slow_s else ("normal" if chair5_s else "")),
        ("baja" if (smm_pct is not None and smm_pct < T.smmwt_low_pct) else ("normal" if smm_pct is not None else "")),
        (calf["label"] if calf_cm else ""),
        ("Positivo" if sarcf["risk"] else "Negativo"),
    ],
}
...
st.markdown(f"**Diagnóstico global:** *{overall}*")
...
informe.append(f"- Edad: {age_in} años | Talla: {pf(height_cm)} cm | Peso: {pf(weight_kg)} kg")
informe.append("")
informe.append("Resultados clave:")
informe.append(f"- Fuerza prensil: {pf(hand_kg)} kg ({strength['label'] if hand_kg>0 else '—'})")
if perc is not None:
    informe.append(f"- Percentil de fuerza: {perc:.1f}%")
if gait_speed is not None:
    informe.append(f"- Velocidad 4 m: {gait_speed:.2f} m/s ({gait_label})")
if chair5_s:
    informe.append(f"- Silla-5: {chair5_s:.1f} s")
if smm_pct is not None:
    informe.append(f"- SMM/peso: {smm_pct:.1f}%")
if calf_cm:
    informe.append(f"- Pantorrilla: {pf(calf_cm)} cm ({calf['label']})")
informe.append(f"- SARC-F: {sarcf['score']}/10 → {'positivo' if sarcf['risk'] else 'negativo'}")
informe.append("")
informe.append(f"Diagnóstico global: {overall}")
informe_txt = "\n".join(informe)

st.divider()
st.subheader("Copiar / descargar informe")
st.text_area("Informe clínico (selecciona y copia):", informe_txt, height=220)
